...
def test_container_has_expected_startup_matches_inspected_cmd(tmp_path: Path):
    ...
    inspect_cmd = Mock(returncode=0, stdout='["-c", "bash /workspace/vllm-hust-dev-hub/scripts/ascend-container-runtime.sh"]', stderr="")
...
def test_install_container_creates_container_when_missing(tmp_path: Path):
    ...
    assert docker_args[-2:] == ["-c", "bash /workspace/demo/scripts/ascend-container-runtime.sh"]
